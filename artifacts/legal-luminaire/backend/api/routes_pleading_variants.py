...
def generate(body: GenerateIn):
    ...
    # Probe citations if the gate service is available
    blocked = []
    try:
        from services.factfit_gate import probe_citations
        report = probe_citations(body.base_draft_md)
        blocked = [c for c in report.get("citations", []) if c["tier"] in ("PENDING", "FATAL_ERROR")]
    except ImportError:
        pass  # gate service not yet wired — skip block in dev
    ...
    try:
        ...
        for h in hits:
            ...
            out.append({
                ...
                "snippet": h["snippet"],
                "tier": tier,
                "insertable": tier in ("COURT_SAFE", "VERIFIED"),
            })
        return {"results": out}
    except ImportError:
        return {"results": []}
