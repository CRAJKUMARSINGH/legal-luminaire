...
export function usePleadingVariants(caseId: string, subject: string) {
  ...
  const spawnVariant = useCallback(
    (baseDraftMd: string, archetype: Archetype) => {
      ...
      const idx = chain.length // snapshot index before stream

      abortRef.current = variantsApi.generateStream(
        { base_draft_md: baseDraftMd, case_id: caseId, subject, archetype_id: archetype.id },
        (delta) =>
          setChain((c) => c.map((n, i) => (i === idx ? { ...n, content: n.content + delta } : n))),
        () => setChain((c) => c.map((n, i) => (i === idx ? { ...n, status: 'done' } : n))),
        (err) => {
          ...
          if (code === 'GATE_BLOCK') {
            ...
            setChain((c) => c.map((n, i) => (i === idx ? { ...n, status: 'blocked' } : n)))
          } else {
            setChain((c) => c.map((n, i) => (i === idx ? { ...n, status: 'error' } : n)))
          }
        },
      )
    },
   ...
   *  (server also enforces). Replaces the first unfilled [[CITATION:n]] slot. */
