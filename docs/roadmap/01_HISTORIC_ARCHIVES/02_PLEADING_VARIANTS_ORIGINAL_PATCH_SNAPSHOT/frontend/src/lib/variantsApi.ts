...
export const variantsApi = {
  subjects: () =>
    fetch(`${BASE}/variants/subjects`).then(json<{ id: string; label: Bilingual }[]>),

  family: (subject: string) =>
    fetch(`${BASE}/variants/families?subject=${encodeURIComponent(subject)}`).then(json<VariantFamily>),

  matterContext: (base_draft_md: string, caseId: string, subject: string) =>
    fetch(`${BASE}/variants/matter-context`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ base_draft_md, case_id: caseId, subject }),
    }).then(json<MatterContext>),
  ...
  generateStream: (
    ...
    ;(async () => {
      try {
        const res = await fetch(`${BASE}/variants/generate`, {
          method: 'POST',
  ...
  citationSearch: (q: string, limit = 10) =>
    fetch(`${BASE}/citations/search`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ q, limit }),
    }).then(json<{ results: CitationHit[] }>),
}
